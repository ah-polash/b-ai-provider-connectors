...
class BAIOAP_Admin {
	...
	public function register_menu() {
		add_options_page(
			__( 'AI Providers', 'b-ai-provider-connectors' ),
			__( 'AI Providers', 'b-ai-provider-connectors' ),
			'manage_options',
			self::MENU_SLUG,
	...
	public function action_links( $links ) {
		array_unshift( $links, '<a href="' . esc_url( $this->page_url() ) . '">' . esc_html__( 'Settings', 'b-ai-provider-connectors' ) . '</a>' );
		return $links;
	}
	...
	public function maybe_enqueue( $hook_suffix ) {
		...
		wp_localize_script(
			...
			array(
				...
				'i18n'          => array(
					'hide' => __( 'Hide videos', 'b-ai-provider-connectors' ),
					'show' => __( 'Show videos', 'b-ai-provider-connectors' ),
				),
			)
		);
	}
	...
	public function render_page() {
		...
		$tabs = array(
			'providers' => array( __( 'Providers', 'b-ai-provider-connectors' ), 'dashicons-cloud' ),
			'custom'    => array( __( 'Custom providers', 'b-ai-provider-connectors' ), 'dashicons-admin-links' ),
			'priority'  => array( __( 'Priority', 'b-ai-provider-connectors' ), 'dashicons-sort' ),
		);
		?>
		<div class="wrap baioap">
			<h1 class="screen-reader-text"><?php esc_html_e( 'AI Providers', 'b-ai-provider-connectors' ); ?></h1>
			...
			<header class="baioap-header">
				<div class="baioap-header__brand">
					...
					<div>
						<div class="baioap-header__title"><?php esc_html_e( 'AI Provider Connectors', 'b-ai-provider-connectors' ); ?> <span class="baioap-pill baioap-pill--muted">v<?php echo esc_html( BAIOAP_VERSION ); ?></span></div>
						<div class="baioap-header__sub"><?php esc_html_e( 'Switch on the providers you want, connect each one with an API key, and decide which provider WordPress tries first.', 'b-ai-provider-connectors' ); ?></div>
					</div>
				</div>
				<div class="baioap-header__actions">
					<a class="baioap-btn baioap-btn--ghost" href="<?php echo esc_url( $this->connectors_url() ); ?>"><span class="dashicons dashicons-admin-network"></span> <?php esc_html_e( 'Manage API keys', 'b-ai-provider-connectors' ); ?></a>
					<a class="baioap-btn baioap-btn--ghost" href="https://wordpress.org/support/plugin/b-ai-provider-connectors/" target="_blank" rel="noopener"><span class="dashicons dashicons-sos"></span> <?php esc_html_e( 'Support', 'b-ai-provider-connectors' ); ?></a>
				</div>
			</header>
			...
			<div class="baioap-layout">
				<nav class="baioap-nav baioap-tabs" aria-label="<?php esc_attr_e( 'AI Providers sections', 'b-ai-provider-connectors' ); ?>">
					<div class="baioap-nav__brand">
						...
						<span><?php esc_html_e( 'AI Providers', 'b-ai-provider-connectors' ); ?></span>
					</div>
					<div class="baioap-nav__group"><?php esc_html_e( 'Sections', 'b-ai-provider-connectors' ); ?></div>
					...
					<div class="baioap-nav__group"><?php esc_html_e( 'At a glance', 'b-ai-provider-connectors' ); ?></div>
					<div class="baioap-stats" id="baioap-stats">
						<span class="baioap-stat"><strong data-stat="enabled"><?php echo (int) $enabled_count; ?></strong> <?php esc_html_e( 'enabled', 'b-ai-provider-connectors' ); ?></span>
						<span class="baioap-stat"><strong data-stat="keyed"><?php echo (int) $keyed_count; ?></strong> <?php esc_html_e( 'with an API key', 'b-ai-provider-connectors' ); ?></span>
						<span class="baioap-stat"><strong><?php echo (int) count( $custom ); ?></strong> / <?php echo (int) BAIOAP_Catalog::CUSTOM_SLOTS; ?> <?php esc_html_e( 'custom', 'b-ai-provider-connectors' ); ?></span>
					</div>
				</nav>
					...
					<section class="baioap-panel baioap-tab" data-tab="providers" <?php echo 'providers' === $tab ? '' : 'hidden'; ?>>
						...
						<div class="baioap-panel__head">
							<div class="baioap-panel__title">
								...
								<div>
									<h2><?php esc_html_e( 'Providers', 'b-ai-provider-connectors' ); ?></h2>
									<p><?php esc_html_e( 'Disabled providers are hidden from the Connectors screen and never used by AI features. Keys you have already saved are kept.', 'b-ai-provider-connectors' ); ?></p>
								</div>
							</div>
						</div>
						...
						<?php if ( $builtin ) : ?>
							<div class="baioap-group">
								<div class="baioap-group__head">
									<h3><?php esc_html_e( 'Built-in providers', 'b-ai-provider-connectors' ); ?></h3>
									<p><?php esc_html_e( 'Registered by WordPress or other plugins. Shown here so you can test them and see their status in one place.', 'b-ai-provider-connectors' ); ?></p>
								</div>
							</div>
					...
					<section class="baioap-panel baioap-tab" data-tab="custom" <?php echo 'custom' === $tab ? '' : 'hidden'; ?>>
						<div class="baioap-panel__head">
							<div class="baioap-panel__title">
								...
								<div>
									<h2><?php esc_html_e( 'Custom providers', 'b-ai-provider-connectors' ); ?></h2>
									<p><?php esc_html_e( 'Connect any OpenAI-compatible API — a hosted service, or a local server such as Ollama, LM Studio or vLLM. After adding it, save its API key on the Connectors screen.', 'b-ai-provider-connectors' ); ?></p>
								</div>
							</div>
							<div class="baioap-toolbar__actions">
								<span class="baioap-slots" id="baioap-slots"><?php echo esc_html( sprintf( /* translators: 1: used slots, 2: total slots. */ __( '%1$d of %2$d used', 'b-ai-provider-connectors' ), count( $custom ), BAIOAP_Catalog::CUSTOM_SLOTS ) ); ?></span>
								<button type="button" class="button button-primary baioap-btn baioap-btn--primary" id="baioap-add-custom" <?php disabled( count( $custom ) >= BAIOAP_Catalog::CUSTOM_SLOTS ); ?>><?php esc_html_e( 'Add provider', 'b-ai-provider-connectors' ); ?></button>
							</div>
						</div>

						<div class="baioap-formpanel" id="baioap-custom-panel" hidden>
							<form id="baioap-custom-form" class="baioap-form" novalidate>
								<h2 class="baioap-formpanel__title baioap-field--full" data-role="panel-title"><?php esc_html_e( 'Add a custom provider', 'b-ai-provider-connectors' ); ?></h2>
								<div class="baioap-field">
									<label for="baioap-f-name"><?php esc_html_e( 'Name', 'b-ai-provider-connectors' ); ?></label>
									<input type="text" id="baioap-f-name" name="name" maxlength="60" required autocomplete="off" placeholder="<?php esc_attr_e( 'e.g. Ollama (local)', 'b-ai-provider-connectors' ); ?>" />
									<p class="baioap-field__error" data-role="error"></p>
								</div>
								<div class="baioap-field">
									<label for="baioap-f-id"><?php esc_html_e( 'ID', 'b-ai-provider-connectors' ); ?></label>
									<input type="text" id="baioap-f-id" name="id" maxlength="40" required autocomplete="off" pattern="[a-z0-9][a-z0-9_-]{1,39}" />
									<p class="description"><?php esc_html_e( 'Lowercase letters, numbers, hyphens. Cannot be changed later — API keys are stored under it.', 'b-ai-provider-connectors' ); ?></p>
									<p class="baioap-field__error" data-role="error"></p>
								</div>
								<div class="baioap-field baioap-field--full">
									<label for="baioap-f-base-url"><?php esc_html_e( 'Base URL', 'b-ai-provider-connectors' ); ?></label>
									<input type="url" id="baioap-f-base-url" name="base_url" required autocomplete="off" placeholder="https://api.example.com/v1" />
									<p class="description"><?php esc_html_e( 'The root of the OpenAI-compatible API. The plugin calls {base URL}/models and {base URL}/chat/completions.', 'b-ai-provider-connectors' ); ?></p>
									<p class="baioap-field__error" data-role="error"></p>
								</div>
								<div class="baioap-field">
									<label for="baioap-f-credentials-url"><?php esc_html_e( 'Where to get an API key (optional)', 'b-ai-provider-connectors' ); ?></label>
									<input type="url" id="baioap-f-credentials-url" name="credentials_url" autocomplete="off" placeholder="https://" />
									<p class="baioap-field__error" data-role="error"></p>
								</div>
								<div class="baioap-field">
									<label for="baioap-f-description"><?php esc_html_e( 'Description (optional)', 'b-ai-provider-connectors' ); ?></label>
									<input type="text" id="baioap-f-description" name="description" maxlength="160" autocomplete="off" />
									<p class="baioap-field__error" data-role="error"></p>
								</div>
								<div class="baioap-form__actions">
									<button type="submit" class="button button-primary baioap-btn baioap-btn--primary" data-role="submit"><?php esc_html_e( 'Save provider', 'b-ai-provider-connectors' ); ?></button>
									<button type="button" class="button baioap-btn" data-role="cancel"><?php esc_html_e( 'Cancel', 'b-ai-provider-connectors' ); ?></button>
						...
						<div class="baioap-empty" id="baioap-custom-empty" <?php echo $custom ? 'hidden' : ''; ?>>
							...
							<h3><?php esc_html_e( 'No custom providers yet.', 'b-ai-provider-connectors' ); ?></h3>
							<p><?php esc_html_e( 'Add one to use a provider that is not bundled, or a model server running on your own hardware.', 'b-ai-provider-connectors' ); ?></p>
						</div>
					</section>
					...
					<section class="baioap-panel baioap-tab" data-tab="priority" <?php echo 'priority' === $tab ? '' : 'hidden'; ?>>
						<div class="baioap-panel__head">
							<div class="baioap-panel__title">
								...
								<div>
									<h2><?php esc_html_e( 'Priority', 'b-ai-provider-connectors' ); ?></h2>
									<p><?php esc_html_e( 'When a feature needs an AI provider, WordPress walks this list from the top and uses the first connected provider that supports the task. Drag to reorder, or use the arrows.', 'b-ai-provider-connectors' ); ?></p>
								</div>
							</div>
						</div>
						<?php if ( empty( $priority ) ) : ?>
							<div class="baioap-empty"><p><?php esc_html_e( 'No AI providers are registered yet.', 'b-ai-provider-connectors' ); ?></p></div>
						<?php else : ?>
							<div class="baioap-group">
								...
								<div class="baioap-priority__actions">
									<button type="button" class="button button-primary baioap-btn baioap-btn--primary" id="baioap-priority-save" disabled><?php esc_html_e( 'Save order', 'b-ai-provider-connectors' ); ?></button>
									<button type="button" class="button baioap-btn" id="baioap-priority-reset" <?php disabled( empty( BAIOAP_Priority::instance()->get_saved_order() ) ); ?>><?php esc_html_e( 'Reset to default', 'b-ai-provider-connectors' ); ?></button>
	...
	public function ajax_toggle_videos() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array( 'message' => __( 'Not allowed.', 'b-ai-provider-connectors' ) ), 403 );
		}
	...
	private function render_videos() {
		...
		$videos = array(
			array(
				...
				'title'  => __( 'See what the plugin does', 'b-ai-provider-connectors' ),
				'copy'   => __( 'A quick look at the bundled providers, custom OpenAI-compatible endpoints and the priority list. The full step-by-step tutorial is coming soon.', 'b-ai-provider-connectors' ),
				'label'  => __( 'Play the video (opens YouTube in this page)', 'b-ai-provider-connectors' ),
				'length' => __( 'Teaser', 'b-ai-provider-connectors' ),
			),
		);
		?>
		<div class="baioap-videos<?php echo $hidden ? ' is-collapsed' : ''; ?>" data-baioap-videos>
			<div class="baioap-videos__bar">
				<div class="baioap-videos__label">
					...
					<span><?php esc_html_e( 'Watch and learn', 'b-ai-provider-connectors' ); ?></span>
					<span class="baioap-pill baioap-pill--soon"><?php esc_html_e( 'Start here', 'b-ai-provider-connectors' ); ?></span>
				</div>
				<button type="button" class="baioap-videos__toggle" data-baioap-videos-toggle
					...
					<span data-baioap-videos-label><?php echo $hidden ? esc_html__( 'Show videos', 'b-ai-provider-connectors' ) : esc_html__( 'Hide videos', 'b-ai-provider-connectors' ); ?></span>
				</button>
			</div>
				...
				<?php foreach ( $videos as $video ) : ?>
					<article class="baioap-vid">
						...
						<div class="baioap-vid__body">
							...
							<a class="baioap-link" href="<?php echo esc_url( 'https://www.youtube.com/watch?v=' . $video['id'] ); ?>" target="_blank" rel="noopener"><?php esc_html_e( 'Watch on YouTube', 'b-ai-provider-connectors' ); ?> &rarr;</a>
						</div>
					</article>
				<?php endforeach; ?>
			</div>

			<p class="baioap-videos__note"<?php echo $hidden ? ' hidden' : ''; ?>><?php esc_html_e( 'The player is not loaded from YouTube until you press play.', 'b-ai-provider-connectors' ); ?></p>
		</div>
		<?php
	}
	...
	private function render_card( array $row ) {
		...
		if ( ! $enabled ) {
			$badge = array( 'muted', __( 'Disabled', 'b-ai-provider-connectors' ) );
		} elseif ( ! $has_key ) {
			$badge = array( 'idle', __( 'No API key', 'b-ai-provider-connectors' ) );
		} else {
			$badge = array( 'info', __( 'Key set', 'b-ai-provider-connectors' ) );
		}
		?>
		...
		>
			<div class="baioap-card__head">
				...
				<div class="baioap-card__title">
					<h3>
						...
						<?php if ( 'custom' === $row['source'] ) : ?>
							<span class="baioap-chip"><?php esc_html_e( 'Custom', 'b-ai-provider-connectors' ); ?></span>
						<?php elseif ( 'builtin' === $row['source'] ) : ?>
							<span class="baioap-chip"><?php esc_html_e( 'Built-in', 'b-ai-provider-connectors' ); ?></span>
						<?php endif; ?>
					</h3>
				</div>
				<?php if ( $is_ours ) : ?>
					<label class="baioap-switch">
						<input type="checkbox" role="switch" class="baioap-switch__input" <?php checked( $enabled ); ?>
							aria-label="<?php echo esc_attr( sprintf( /* translators: %s: provider name. */ __( 'Enable %s', 'b-ai-provider-connectors' ), $row['name'] ) ); ?>" />
			...
			<div class="baioap-card__status">
				...
				<?php if ( 'custom' !== $row['source'] && ! empty( $row['credentialsUrl'] ) ) : ?>
					<a class="baioap-card__link" href="<?php echo esc_url( $row['credentialsUrl'] ); ?>" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Get an API key', 'b-ai-provider-connectors' ); ?> <span aria-hidden="true">&#8599;</span></a>
				<?php endif; ?>
			</div>

			<div class="baioap-card__actions">
				<button type="button" class="button baioap-test" <?php disabled( ! $enabled ); ?>><?php esc_html_e( 'Test connection', 'b-ai-provider-connectors' ); ?></button>
				<a class="button" href="<?php echo esc_url( $this->connectors_url() ); ?>"><?php echo $has_key ? esc_html__( 'Manage key', 'b-ai-provider-connectors' ) : esc_html__( 'Add key', 'b-ai-provider-connectors' ); ?></a>
				<?php if ( 'custom' === $row['source'] ) : ?>
					<span class="baioap-card__links">
						<button type="button" class="button-link baioap-edit"><?php esc_html_e( 'Edit', 'b-ai-provider-connectors' ); ?></button>
						<button type="button" class="button-link button-link-delete baioap-delete"><?php esc_html_e( 'Delete', 'b-ai-provider-connectors' ); ?></button>
					</span>
				<?php endif; ?>
			</div>
		</article>
	...
	private function render_priority_item( array $row, $rank ) {
		...
		<li class="baioap-priority__item" draggable="true" data-id="<?php echo esc_attr( $row['id'] ); ?>">
			...
			<span class="baioap-priority__name"><?php echo esc_html( $row['name'] ); ?>
				<?php if ( 'custom' === $row['source'] ) : ?><span class="baioap-chip"><?php esc_html_e( 'Custom', 'b-ai-provider-connectors' ); ?></span><?php elseif ( 'builtin' === $row['source'] ) : ?><span class="baioap-chip"><?php esc_html_e( 'Built-in', 'b-ai-provider-connectors' ); ?></span><?php endif; ?>
			</span>
			<span class="baioap-badge is-<?php echo $has_key ? 'success' : 'idle'; ?>"><?php echo $has_key ? esc_html__( 'Key set', 'b-ai-provider-connectors' ) : esc_html__( 'No API key', 'b-ai-provider-connectors' ); ?></span>
			<span class="baioap-priority__buttons">
				<button type="button" class="button button-small baioap-up" aria-label="<?php echo esc_attr( sprintf( /* translators: %s: provider name. */ __( 'Move %s up', 'b-ai-provider-connectors' ), $row['name'] ) ); ?>">&#9650;</button>
				<button type="button" class="button button-small baioap-down" aria-label="<?php echo esc_attr( sprintf( /* translators: %s: provider name. */ __( 'Move %s down', 'b-ai-provider-connectors' ), $row['name'] ) ); ?>">&#9660;</button>
			</span>
		</li>
		<?php
	}
