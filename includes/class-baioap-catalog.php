...
class BAIOAP_Catalog {
	...
	public static function validate_custom( array $input, $slot = null ) {
		...
		if ( '' === $name || mb_strlen( $name ) > 60 ) {
			return new WP_Error( 'baioap_invalid_name', __( 'Please enter a name of up to 60 characters.', 'b-ai-provider-connectors' ), array( 'status' => 400, 'field' => 'name' ) );
		}
		...
		if ( $current ) {
			...
			if ( ! preg_match( '/^[a-z0-9][a-z0-9_-]{1,39}$/', $id ) ) {
				return new WP_Error( 'baioap_invalid_id', __( 'The ID must be 2–40 characters: lowercase letters, numbers, hyphens or underscores.', 'b-ai-provider-connectors' ), array( 'status' => 400, 'field' => 'id' ) );
			}
			...
			if ( in_array( $id, $taken, true ) ) {
				return new WP_Error( 'baioap_duplicate_id', __( 'That ID is already used by another provider.', 'b-ai-provider-connectors' ), array( 'status' => 400, 'field' => 'id' ) );
			}
		}
		...
		if ( '' === $base_url || ! wp_http_validate_url( $base_url ) && 0 !== strpos( $base_url, 'http://localhost' ) && 0 !== strpos( $base_url, 'http://127.0.0.1' ) ) {
			return new WP_Error( 'baioap_invalid_url', __( 'Please enter a valid base URL, for example https://api.example.com/v1.', 'b-ai-provider-connectors' ), array( 'status' => 400, 'field' => 'base_url' ) );
		}
			...
			if ( null === $slot ) {
				return new WP_Error(
					...
					sprintf(
						...
						__( 'You can add up to %d custom providers. Remove one to add another.', 'b-ai-provider-connectors' ),
						self::CUSTOM_SLOTS
					),
					array( 'status' => 400 )
				);
